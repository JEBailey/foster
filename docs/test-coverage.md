...
| Bytecode compiler, optimizer, CFG type/ownership verifier, VM, and binary format | Every Foster suite runs optimized and unoptimized bytecode | IR lowering, malformed type/ownership/control-flow programs, deterministic encoding, optimizer reductions, and semantic equivalence |
| Native compilation | Portable primitive programs provide source fixtures | Object emission, supported ABI/results, unsupported reachability, executable output, and native failures |
