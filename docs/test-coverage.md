...
| Closures, nested functions, capture modes, partial application, and callable ownership | `tests/foster/functions.fos`, `tests/foster/effects.fos`, and `library/core/functions.fos` | Creation-time capture ordering and backend parity, capture classification, invalid escapes and invalidation, erased callable representation, and suppression of generated capture locals in LSP results |
...
| Values versus places, stored fields, and computed-member result ownership | `tests/foster/values.fos` and library member tests | Accepted stored assignment, rejected computed assignment, copy/owned/borrowed member metadata, ownership effects, and temporary provenance |
| Operand sequencing and full-expression lifetime boundaries | `tests/foster/control_flow.fos` | Side-effect ordering under VM/native execution, assignment RHS-before-destination, reverse temporary cleanup, early-transfer cleanup, and escaping-temporary rejection |
| Records, generic records, aliases, enums, and enum contracts | `tests/foster/data.fos` and `tests/foster/advanced_types.fos` | Construction errors, privacy, recursive aliases, exhaustiveness, and HIR metadata |
...
| Remote objects, futures, virtual threads, borrowed messages, and persistent read loans | `tests/foster/remote.fos` | Typed failure outcomes, terminal worker state, discarded failures, forbidden mutation/messages, serialization, overload dispatch, and suspension inference |
...
| Copy, Drop, and fallible List reads | Core copy methods and collection suites | `tests/backend_parity.rs` checks user copy dispatch, bounds/capability errors, destructor ordering, replacement, moves, loop exits, remote argument cleanup, and failures in both engines and optimization modes |
| Native compilation | Portable scalar, record, and enum programs provide source fixtures | Object/descriptor emission, supported ABI/results, generic function/method and nominal-layout specialization, record copy-on-write and borrowed calls, enum tags/patterns, generated ownership operations, unsupported reachability, executable output, and allocation-census tests for normal and exceptional cleanup in both optimization modes |
