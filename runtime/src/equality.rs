...
unsafe fn equal_object(left: usize, right: usize) -> bool {
    unsafe {
        ...
        if descriptor != runtime_word(right) {
            return false;
        }
        let mut offset = 6;
        ...
        match kind {
            ...
            1 => {
                ...
                if tag != ((right + tag_offset) as *const u32).read_unaligned() {
                    return false;
                }
                ...
                for _ in 0..alternatives {
                    ...
                    if candidate == tag {
                        return equal_fields(left, right, &fields);
                    }
                }
            ...
            4 | 5 => {
                ...
                if length != runtime_word(right + length_offset) {
                    return false;
                }
            ...
            7 => {
                ...
                runtime_word(left + code_offset) == runtime_word(right + code_offset)
                    && equal_object(
                        runtime_word(left + environment_offset),
                        runtime_word(right + environment_offset),
                    )
            }
            8 => {
